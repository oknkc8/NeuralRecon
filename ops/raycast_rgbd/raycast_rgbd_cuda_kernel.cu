...
struct RayCastParams
{
	...
	__device__ inline
	float getOx() const { return origin[0]; }
	__device__ inline
	float getOy() const { return origin[1]; }
	__device__ inline
	float getOz() const { return origin[2]; }
};
			...
			if(lastSample.weight > 0 && ((lastSample.sdf > 0.0f && dist < 0.0f) || (lastSample.sdf < 0.0f && dist > 0.0f))) // current sample is always valid here 
			{
				...
				if(b && abs(lastSample.sdf - dist) < params.threshSampleDist)
				{
					if(abs(dist) < params.threshSampleDist)
					{
						...
						//printf("currentPosWorld x: %f, y: %f, z: %f  |  depth: %f\n", currentPosWorld.x, currentPosWorld.y, currentPosWorld.z, depth);
...
__global__ void raycast_rgbd_cuda_kernel(
	...
	if (x < params.width && y < params.height /*&& batch < batch_size*/) {
		...
		float3 origin = make_float3(params.getOx(), params.getOy(), params.getOz());

		float3 worldCamPos = ((curViewMatrixInv * make_float3(0.0f, 0.0f, 0.0f)) / params.voxelSize) - (origin / params.voxelSize);
		float4 w = curViewMatrixInv * make_float4(camDir, 0.0f);
		float3 worldDir = normalize(make_float3(w.x, w.y, w.z));

		// printf("origin: %f %f %f   |   worlCamPos_tmp1: %f %f %f  |   worlCamPos_tmp2: %f %f %f  |   worlCamPos_tmp: %f %f %f\n", 
		// 		origin.x, origin.y, origin.z,
		// 		worldCamPos_tmp1.x, worldCamPos_tmp1.y, worldCamPos_tmp1.z,
		// 		worldCamPos_tmp2.x, worldCamPos_tmp2.y, worldCamPos_tmp2.z,
		// 		worldCamPos.x, worldCamPos.y, worldCamPos.z);

		// printf("origin: %f %f %f  |   camDir: %f %f %f   |   worlCamPos: %f %f %f   |   worldDir: %f %f %f \n", 
		// 		origin.x, origin.y, origin.z,
...
void raycast_rgbd_cuda_forward(
	...
	//printf("check\n");
	params.origin = origin.data<float>();
	//printf("check2\n");
	
	//std::cout << origin << std::endl;
	
	//printf("check3\n");
	//std::cout << params.origin[0] << " " << params.origin[1] << " " << params.origin[2] << std::endl;
	params.voxelSize = opts_accessor[9];
	//printf("check4\n");
